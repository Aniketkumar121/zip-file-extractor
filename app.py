...
class ProjectExtractor:
    ...
    def extract_zip_contents_without_comments(self, zip_path, output_file):
        """Extract zip but remove unwanted comments in code files and empty __init__.py files"""
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for root, dirs, files in os.walk(temp_dir):
                ...
                for file in files:
                    ...
                    # Check if the file is '__init__.py' and is empty, remove if so
                    if relative_path.endswith('__init__.py'):
                        if os.path.getsize(file_path) == 0:
                            os.remove(file_path)  # Remove the empty __init__.py file
